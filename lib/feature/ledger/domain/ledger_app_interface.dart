...
class LedgerAppInterface {
  LedgerAppInterface({
    ...
    _deviceModel = ledgerDevices.firstWhere((d) => d.id == deviceModelId);
    _bleService = device.servicesList.firstWhere(
      (e) => e.uuid.toString() == _deviceModel.bluetoothSpec.serviceUuid,
    );
    _writeCharacteristic = _bleService.characteristics.firstWhere(
      (c) => c.uuid.toString() == _deviceModel.bluetoothSpec.writeUuid,
    );
    _notifyCharacteristic = _bleService.characteristics.firstWhere(
      (c) => c.uuid.toString() == _deviceModel.bluetoothSpec.notifyUuid,
    );
  }
  ...
  late final LedgerDeviceModel _deviceModel;
  ...
  Future<Uint8List> getConfiguration() async {
    ...
    try {
      final writer = ByteDataWriter()
        ..writeUint8(_cla) // CLA
        ..writeUint8(_insGetConf) // INS_GET_CONFIG
        ..writeUint8(0x00) // P1
        ..writeUint8(0x00); // P2
  ...
  Future<Uint8List> getPublicKey(int accountId) async {
    ...
    try {
      ...
      final writer = ByteDataWriter()
        ..writeUint8(_cla) // CLA
        ..writeUint8(_insGetPk) // INS_GET_PK
        ..writeUint8(0x00) // P1
        ..writeUint8(0x00) // P2
  ...
  Future<Uint8List> signTransaction({
    ...
    try {
      ...
      for (var i = 0; i < chunks.length; i++) {
        ...
        final writer = ByteDataWriter()
          ..writeUint8(_cla) // CLA
          ..writeUint8(_insSignTransaction) // INS_SIGN_TRANSACTION
          ..writeUint8(0x01) // P1
          ..writeUint8(p2) // P2
