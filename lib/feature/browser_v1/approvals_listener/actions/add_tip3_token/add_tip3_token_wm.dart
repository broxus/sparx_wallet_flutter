...
@injectable
class AddTip3TokenWidgetModel extends InjectedWidgetModel<AddTip3TokenWidget,
    AddTip3TokenModel, AddTip3TokenWmParams> {
  ...
  late final _account = createWmParamsNotifier(
    (it) => it.account,
  );

  late final _details = createWmParamsNotifier(
    (it) => it.details,
  );

  ValueListenable<Address> get accountState => _account;

  ValueListenable<Uri> get origin => _origin;

  ValueListenable<TokenContractAsset> get details => _details;
  ...
  void initWidgetModel() {
    ...
    _subscription =
        model.allAvailableContractsForAccount(_account.value).listen(_onData);
  ...
  Future<void> _getBalance() async {
    final wallet = await model.getTokenWallet(
      owner: _account.value,
      rootTokenContract: _details.value.address,
    );
  ...
  void _onData((List<TokenContractAsset>, List<TokenContractAsset>) value) {
    ...
    final address = _details.value.address;
    final symbol = _details.value.symbol;
