...
class StakingInProgress extends InjectedElementaryParametrizedWidget<
    StakingInProgressWidgetModel, StakingInProgressParams> {
  StakingInProgress({
  ...
  Widget build(StakingInProgressWidgetModel wm) {
    return SeparatedColumn(
      children: wm.requests.mapIndexed(
        (index, e) {
          final prev = index == 0 ? null : wm.requests[index - 1];
          final next =
              index == wm.requests.length - 1 ? null : wm.requests[index + 1];
          final isFirst =
