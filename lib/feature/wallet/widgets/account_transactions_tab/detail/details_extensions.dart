...
Money _getNativeMoney(BigInt value) => Money.fromBigIntWithCurrency(
  value,
  Currencies()[inject<NekotonRepository>().currentTransport.nativeTokenTicker]!,
);
...
Money _getTokenMoney(BigInt value, Address tokenAddress) {
  final possibleTokenSymbol = inject<AssetsService>()
      ...
      ?.symbol;
  final currency = possibleTokenSymbol == null
      ? null
      : Currencies()[possibleTokenSymbol];
...
extension KnownPayloadX on KnownPayload {
  ...
  DetailsTitleAndBody? toRepresentableData() => switch (this) {
    KnownPayloadComment(:final data) =>
      data.isNotEmpty
          ? (
      ...
      [
        ...
        TonWalletTransactionDetailsItem(
          ...
          contentChild: MoneyWidget(
            money: _getTokenMoney(data.tokens, switch (data.to) {
    ...
    KnownPayloadTokenSwapBack(:final data) => (
      ...
      [
        TonWalletTransactionDetailsItem(
          title: LocaleKeys.tokensWord.tr(),
          contentChild: MoneyWidget(
            money: _getTokenMoney(data.tokens, data.callbackAddress),
            style: MoneyWidgetStyle.primary,
            showSymbol: false,
          ),
        ),
