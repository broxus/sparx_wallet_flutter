...
class WalletDeployConfirmWidgetModel
  ...
  static final _logger = Logger('WalletDeployConfirmWidgetModel');

  late final deployTypeState = createWmParamsNotifier(
    (params) => params.deployType,
  );
  late final custodiansState = createWmParamsNotifier(
    (params) => params.custodians,
  );
  late final requireConfirmationsState = createWmParamsNotifier(
    (params) => params.requireConfirmations,
  );
  late final publicKeyState = createWmParamsNotifier(
    (params) => params.publicKey,
  );

  late final _feeState = createNotifier<BigInt>();
  late final _balanceState = createNotifier<BigInt>();
  late final _accountState = createNotifier<KeyAccount>();
  late final _isLoadingState = createValueNotifier(true);
  late final _errorMessageState = createValueNotifier<String?>(null);
  late final _hasSufficientBalanceState = createValueNotifier(false);
  late final _currencyState = createNotifier<CustomCurrency>();
  late final _walletState = createNotifier<TonWallet>();
  ...
  Future<void> _fetchNativeCurrency() async {
    try {
      ...
      _logger.severe('_fetchNativeCurrency', e, s);
    }
  }
