...
class ExponentialBackoff {
  ...
  ExponentialBackoff({
    ...
    this.maxDelay,
    this.jitter = 0.0,
  }) : assert(multiplier >= 1.0, 'Multiplier must be greater than or equal to 1.0'),
  ...
  Duration _calculateNextDelay(Duration currentDelay) {
    ...
    if (jitter > 0 && _random != null) {
      final jitterMs = (nextDelay.inMilliseconds * jitter * _random.nextDouble()).toInt();
  ...
  Future<T> run<T>(
    ...
    void Function(Object exception, StackTrace stackTrace, int attempt)? onException,
