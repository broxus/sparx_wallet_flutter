...
void main() {
  ...
  group('findLatestVersion', () {
    test('returns null when release notes are null', () {
      // Act
      final result = latestVersionFinder.findLatestVersion(null, '1.0.0');
    ...
    test(
      'returns the latest available version when current version is lower',
      () {
        // Arrange
        const releaseNotes = ReleaseNotes(
          notes: {
    ...
    test(
      'returns null when current version is greater than latest available',
      () {
        // Arrange
        const releaseNotes = ReleaseNotes(
          notes: {
    ...
    test(
      'selects version with highest version number when multiple available',
      () {
        // Arrange
        const releaseNotes = ReleaseNotes(
          notes: {
