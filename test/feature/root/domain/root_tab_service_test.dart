...
void main() {
  ...
  setUp(() {
    ...
    rootTabService = RootTabService(mockRouter, mockBrowserService);
  ...
  group('RootTabService', () {
    group('rootTabStream', () {
      test('should emit wallet tab when current route is WalletRoute', () {
        ...
        when(
          () => mockRouter.currentRoutesStream,
        ).thenAnswer((_) => routesStream.stream);

        // Act & Assert
        expect(rootTabService.rootTabStream, emits(RootTab.wallet));
      });
      ...
      test('should emit browser tab when current route is BrowserRoute', () {
        ...
        // Act & Assert
        expect(rootTabService.rootTabStream, emits(RootTab.browser));
      });
      ...
      test('should emit profile tab when current route is ProfileRoute', () {
        ...
        // Act & Assert
        expect(rootTabService.rootTabStream, emits(RootTab.profile));
      });
      ...
      test('should emit multiple tabs as routes change', () {
        ...
        expect(
          ...
          emitsInOrder([RootTab.wallet, RootTab.browser]),
        );
    ...
    group('bottomBarStateStream', () {
      test(
        ...
        () {
          ...
          when(
            () => mockBrowserService.isContentInteractedStream,
          ).thenAnswer((_) => scrolledStream.stream);
      ...
      test(
        ...
        () async {
          ...
          expect(
            rootTabService.bottomBarStateStream,
            emitsInOrder([BottomBarState.expanded, BottomBarState.collapsed]),
          );
      ...
      test('should emit expanded state for non-BrowserRoute '
    ...
    group('tryToChangeTabAndCheckDiff', () {
      test(
        'should return true and emit tab when current tab differs',
        () async {
          // Arrange
          final walletRoute = MockWalletRoute();
          ...
          final subscription = rootTabService.scrollTabToTopSubject.listen((
            tab,
          ) {
            tabEmitted = true;
          });
          ...
          // Act
          final result = rootTabService.tryToChangeTabAndCheckDiff(
            RootTab.browser,
          );
      ...
      test(
        'should return false and not emit when current tab is same',
        () async {
          // Arrange
          final walletRoute = MockWalletRoute();
          when(() => mockRouter.currentRoutes).thenReturn([walletRoute]);
